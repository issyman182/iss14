...
public sealed partial class ImmovableRodSystem : EntitySystem
{
    ...
    private void OnCollide(EntityUid uid, ImmovableRodComponent component, ref StartCollideEvent args)
    {
        ...
        if (TryComp<BodyComponent>(ent, out var body))
        {
            ...
            _adminLogger.Add(LogType.Gib, LogImpact.Low, $"Entity {ToPrettyString(uid)} gibbed {ToPrettyString(ent)} at X:{gibCoords.X} Y:{gibCoords.Y}");

            _bodySystem.GibBody(ent, body: body); // iss14: classic body gib
