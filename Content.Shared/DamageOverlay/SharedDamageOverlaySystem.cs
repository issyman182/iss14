...
public abstract partial class SharedDamageOverlaySystem : EntitySystem
{
    ...
    // iss14: the fork's MobThresholdChecked is a networked entity-only event, so react to the local
    // counterpart and let UpdateOverlays resolve the components itself.
    [SubscribeLocalEvent]
    private void OnThresholdCheck(Entity<DamageOverlayComponent> entity, ref MobThresholdsChecked args)
    {
        UpdateOverlays(entity);
    }
