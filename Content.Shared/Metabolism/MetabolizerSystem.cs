...
public sealed partial class MetabolizerSystem : EntitySystem
{
    ...
    // (see Content.Server BodySystem.OnApplyMetabolicMultiplier), so handle the plain event.
    [SubscribeLocalEvent]
    ...
    {
        // iss14: fork's organ API - returns Entity<T, OrganComponent>, so the metabolizer is Comp1
        var organs = _body.GetBodyOrganEntityComps<MetabolizerComponent>(entity);

        foreach (var organ in organs)
        {
            TryAddMetabolizerType((organ.Owner, organ.Comp1), metabolizer);
        }
    }
    ...
    {
        // iss14: classic/Shitmed body system - GetBodyOrganEntityComps returns Entity<T, OrganComponent>.
        var metabolisms = _body.GetBodyOrganEntityComps<MetabolizerComponent>(targetBody);
        foreach (var organ in metabolisms)
        {
            if (HasMetabolizer((organ.Owner, organ.Comp1), targetMetabolizer))
                return true;
        }
