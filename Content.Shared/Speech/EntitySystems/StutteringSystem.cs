...
public sealed partial class StutteringSystem : RelayAccentSystem<StutteringAccentComponent>
{
    ...
    /// <summary>
    /// iss14: subtracts from the stuttering status effect, mirroring
    /// <see cref="Drunk.SharedDrunkSystem.TryRemoveDrunkennessTime"/>. Used by the bloodstream to undo
    /// the stutter it applied at low blood level.
    /// </summary>
    [PublicAPI]
    public void DoRemoveStutterTime(EntityUid uid, TimeSpan time)
    {
        _statusEffects.TryAddTime(uid, StutterEffect, -time);
    }
}
