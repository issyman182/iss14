...
public sealed partial class BibleSystem : EntitySystem
{
    ...
    private void OnAfterInteract(Entity<BibleComponent> ent, ref AfterInteractEvent args)
    {
        ...
        if (!HasComp<BibleUserComponent>(args.User))
        {
            ...
            _damageable.TryChangeDamage(args.User, ent.Comp.DamageOnUntrainedUse, true, origin: ent, targetPart: TargetBodyPart.All, ignoreBlockers: true); // Shitmed Change
        ...
        // Shitmed Change - heal every part; the part-aware overload returns the applied damage
        var healed = _damageable.TryChangeDamage(args.Target.Value, ent.Comp.Damage, true, origin: ent, targetPart: TargetBodyPart.All, ignoreBlockers: true, splitDamage: SplitDamageBehavior.SplitEnsureAll);
        if (healed != null && !healed.Empty)
        {
